...
public final class QuadUtil {
	...
	public static void assignLerpedUVs(MutableQuadView quad, TextureAtlasSprite sprite) {
		AnimationMetadataSection animMeta = sprite.animationMetadata;

		float delta = 0f;
		if (animMeta != null) {
			int currentFrame = sprite.frameCounter;
			int frameDuration = animMeta.getFrameTimeSingle(currentFrame);
			if (frameDuration > 0) {
				delta = (float) sprite.tickCounter / (float) frameDuration;
			}
		}

		float centerU = (sprite.getMinU() + sprite.getMaxU()) * 0.5f;
		float centerV = (sprite.getMinV() + sprite.getMaxV()) * 0.5f;
		float lerpedMinU = MathUtil.lerp(delta, sprite.getMinU(), centerU);
		float lerpedMaxU = MathUtil.lerp(delta, sprite.getMaxU(), centerU);
		float lerpedMinV = MathUtil.lerp(delta, sprite.getMinV(), centerV);
		float lerpedMaxV = MathUtil.lerp(delta, sprite.getMaxV(), centerV);
	...
	public static void emitOverlayQuad(QuadEmitter emitter, EnumFacing face, TextureAtlasSprite sprite, int color, RenderMaterial material) {

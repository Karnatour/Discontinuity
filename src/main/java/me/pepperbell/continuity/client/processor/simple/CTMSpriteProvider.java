...
public class CTMSpriteProvider implements SpriteProvider {
	...
	public CTMSpriteProvider(TextureAtlasSprite[] sprites, ConnectionPredicate connectionPredicate, boolean innerSeams, boolean useTextureOrientation) {
	...
	public TextureAtlasSprite getSprite(QuadView quad, TextureAtlasSprite sprite, World blockView, IBlockState state, BlockPos pos, Supplier<Random> randomSupplier, ProcessingDataProvider dataProvider) {
		EnumFacing[] directions = useTextureOrientation ? DirectionMaps.getDirections(quad) : DirectionMaps.getMap(quad.lightFace())[0];
	...
	public static int getConnections(ConnectionPredicate connectionPredicate, boolean innerSeams, EnumFacing[] directions, BlockPos.MutableBlockPos mutablePos, World blockView, IBlockState state, BlockPos pos, EnumFacing face, TextureAtlasSprite quadSprite) {
		int connections = 0;
		for (int i = 0; i < 4; i++) {
	...
	public static class Factory implements SpriteProvider.Factory<StandardConnectingCTMProperties> {
		...
		@Override
		public SpriteProvider createSpriteProvider(TextureAtlasSprite[] sprites, StandardConnectingCTMProperties properties) {

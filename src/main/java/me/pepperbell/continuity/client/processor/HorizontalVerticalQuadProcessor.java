...
public class HorizontalVerticalQuadProcessor extends HorizontalQuadProcessor {
	...
	public HorizontalVerticalQuadProcessor(TextureAtlasSprite[] sprites, ProcessingPredicate processingPredicate, ConnectionPredicate connectionPredicate, boolean innerSeams) {
	...
	protected int getSecondaryConnections(EnumFacing[] directions, BlockPos.MutableBlockPos mutablePos, World blockView, IBlockState state, BlockPos pos, EnumFacing face, TextureAtlasSprite quadSprite) {
		int connections = 0;
		for (int i = 0; i < 2; i++) {
			EnumFacing direction = directions[i * 2 + 1];
			...
			if (connectionPredicate.shouldConnect(blockView, state, pos, mutablePos, face, quadSprite, innerSeams)) {
				connections |= 1 << (i * 3 + 1);
				for (int j = 0; j < 2; j++) {
					mutablePos.setPos(pos).move(direction).move(directions[((i + j) % 2) * 2]);
					if (connectionPredicate.shouldConnect(blockView, state, pos, mutablePos, face, quadSprite, innerSeams)) {

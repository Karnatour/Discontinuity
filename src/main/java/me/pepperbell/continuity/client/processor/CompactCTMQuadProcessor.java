...
public class CompactCTMQuadProcessor extends ConnectingQuadProcessor {
	...
	public CompactCTMQuadProcessor(TextureAtlasSprite[] sprites, ProcessingPredicate processingPredicate, ConnectionPredicate connectionPredicate, boolean innerSeams, TextureAtlasSprite[] replacementSprites) {
	...
	public ProcessingResult processQuadInner(MutableQuadView quad, TextureAtlasSprite sprite, World blockView, IBlockState state, BlockPos pos, Supplier<Random> randomSupplier, int pass, int processorIndex, ProcessingContext context) {
		...
		EnumFacing[] directions = DirectionMaps.getMap(quad.lightFace())[orientation];
		...
		if (uSplit & vSplit) {
			...
			if (split01 & split12 & split23 & split30) {
				...
				if (uSplit01) {
					...
					delta4 = MathUtil.getLerpProgress(0, MathUtil.lerp(delta01, vn0, vn1), MathUtil.lerp(delta23, vn2, vn3));
				} else {
					...
					delta4 = MathUtil.getLerpProgress(0, MathUtil.lerp(delta01, un0, un1), MathUtil.lerp(delta23, un2, un3));
				}
				...
				if (splits == 2) {
					...
					if (!split01) {
						...
						if (uSplit01) {
							...
							delta12 = MathUtil.getLerpProgress(0, vn1, vn2);
							delta30 = MathUtil.getLerpProgress(0, vn3, vn0);
							delta4 = MathUtil.getLerpProgress(0, MathUtil.lerp(delta12, un1, un2), MathUtil.lerp(delta30, un3, un0));
						} else {
							delta23 = MathUtil.getLerpProgress(0, vn2, vn3);
							delta12 = MathUtil.getLerpProgress(0, un1, un2);
							delta30 = MathUtil.getLerpProgress(0, un3, un0);
							delta4 = MathUtil.getLerpProgress(0, MathUtil.lerp(delta12, vn1, vn2), MathUtil.lerp(delta30, vn3, vn0));
						}
	...
	protected void splitHalf(QuadView quad, TextureAtlasSprite sprite, RenderMaterial material, VertexContainer vertexContainer, int id, QuadEmitter quadEmitter, int spriteIndex) {
	...
	protected void splitQuadrant(QuadView quad, TextureAtlasSprite sprite, RenderMaterial material, VertexContainer vertexContainer, int id, QuadEmitter quadEmitter, int spriteIndex) {
		quad.copyTo(quadEmitter);
		quadEmitter.material(material);
	...
	public static class Vertex {
		...
		public void setLerped(float delta, Vertex vertexA, Vertex vertexB) {
			x = MathUtil.lerp(delta, vertexA.x, vertexB.x);
			y = MathUtil.lerp(delta, vertexA.y, vertexB.y);
			z = MathUtil.lerp(delta, vertexA.z, vertexB.z);
			color = MathUtil.lerpColor(delta, vertexA.color, vertexB.color);
			light = MathUtil.lerpLight(delta, vertexA.light, vertexB.light);
			u = MathUtil.lerp(delta, vertexA.u, vertexB.u);
			v = MathUtil.lerp(delta, vertexA.v, vertexB.v);
			if (vertexA.hasNormal && vertexB.hasNormal) {
				normalX = MathUtil.lerp(delta, vertexA.normalX, vertexB.normalX);
				normalY = MathUtil.lerp(delta, vertexA.normalY, vertexB.normalY);
				normalZ = MathUtil.lerp(delta, vertexA.normalZ, vertexB.normalZ);
	...
	public static class Factory implements QuadProcessorFactory<CompactConnectingCTMProperties> {
		@Override
		public QuadProcessor createProcessor(CompactConnectingCTMProperties properties, Function<ResourceLocation, TextureAtlasSprite> textureGetter) {
		...
		public QuadProcessor createProcessor(CompactConnectingCTMProperties properties, TextureAtlasSprite[] sprites, TextureAtlasSprite[] replacementSprites) {

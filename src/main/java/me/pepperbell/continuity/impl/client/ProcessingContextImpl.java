...
public class ProcessingContextImpl implements QuadProcessor.ProcessingContext {
	...
	public void accept(RenderContext context) {
		if (!emitterConsumers.isEmpty()) {
            ...
            for (Consumer<QuadEmitter> emitterConsumer : emitterConsumers) {
                emitterConsumer.accept(quadEmitter);
            }
		}
		if (!meshes.isEmpty()) {
	...
	protected void resetData() {
        ...
        for (ProcessingDataKey<?> processingDataKey : allResettable) {
            resetData(processingDataKey);
        }
	}
